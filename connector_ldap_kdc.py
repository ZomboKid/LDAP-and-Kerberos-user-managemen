...
parser = argparse.ArgumentParser(usage="\n\rBefore using the utility, \
first configure the configuration YAML file ./connector_ldap_kdc_conf.yaml\
\n\rExample:\n\rLDAP_URL: \"ldap:///\"\n\rLDAP_ADM: \"cn=admin,dc=test,\
dc=local\"\n\rLDAP_ADM_PASSW: \"passw0rd\"\n\rDC: \"dc=test,dc=local\"\
\n\rREALM: \"TEST.LOCAL\"\n\rPRINCIPAL_OU: \"cn=TEST.LOCAL,cn=kerberos,\
ou=kdcroot\"\n\r\n\r-add/--createkrb <USERNAME> <NEWPASS> - adding \
principal\n\rExample: ./connector_ldap_kdc -add ivanov qwerty123\n\r\n\r\
-chpasskrb <USERNAME> <NEWPASS> - changing principal's password\n\r\
Example: ./connector_ldap_kdc -chpasskrb ivanov passw0rd\n\r\n\r-chpass \
<USERNAME> <OLDPASS> <NEWPASS> - changing LDAP user's password and \
principal's password\n\rExample: ./connector_ldap_kdc.py -chpass ivanov \
qwerty123 passw0rd\n\r\n\r-lock/--lockkrb <USERNAME> - locking user\n\r\
Example: ./connector_ldap_kdc -lock ivanov\n\r\n\r-unlock/--unlockkrb \
<USERNAME> - unlocking user\n\rExample: ./connector_ldap_kdc -unlock ivanov")

parser.add_argument("-add", "--createkrb", nargs=2, type=str, action="store",
                    help=argparse.SUPPRESS)
parser.add_argument("-chpasskrb", "--chpasskrb", nargs=2, type=str,
                    action="store", help=argparse.SUPPRESS)
parser.add_argument("-chpass", "--chpass", nargs=3, type=str,
                    action="store", help=argparse.SUPPRESS)
parser.add_argument("-lock", "--lockkrb", nargs=1, type=str,
                    action="store", help=argparse.SUPPRESS)
parser.add_argument("-unlock", "--unlockkrb", nargs=1, type=str,
                    action="store", help=argparse.SUPPRESS)
# -------------------------------------------------------------------------------------------------------------------------------------------
if len(sys.argv) < 2:
...
def f_lock(USERNAME):
    ...
    mod_attrs = [(ldap.MOD_REPLACE, 'krbLoginFailedCount', str(10)),
                 (ldap.MOD_REPLACE, 'krbLastFailedAuth', krb_field)]
